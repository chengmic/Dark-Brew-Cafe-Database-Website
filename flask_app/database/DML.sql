...
SELECT customer_id, CONCAT(Customers.first_name, ' ', Customers.last_name) FROM Customers
...
SELECT order_details_id, Orders.order_id, Items.item_name, item_quantity, item_total
FROM Order_Details
...
-- Get a single Order's data for the Update Order form
SELECT order_details_id, order_id, item_id, item_quantity, item_total FROM Order_Details
WHERE order_details_id = :order_details_ID_selected_from_browse_order_details_page;
...
WHERE :order_details_ID_selected_from_browse_order_page;
...
-- Update unit_cost column (to be used in procedure)
UPDATE Order_Details
	SET unit_cost = (
	SELECT item_price 
	FROM Items
...
-- Update item_total column (to be used in procedure)
UPDATE Order_Details
SET item_total = (
